...
test('reports the final result without any test cases formatted as TAP', async () => {
...
test('reports the final result with succeeded and failed test cases formatted as TAP', async () => {
    const log = sinon.fake();
    const reporter = tapConsoleReporterFactory({ log });
    const session = reporter.createSession(42);
